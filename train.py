...
def configurations():
  ...
  # train_config = configuration.TRAIN_CONFIG_MVD
  train_config = configuration.TRAIN_CONFIG_AVMP
...
def main(model_config, train_config):
  ...
  with g.as_default():
    ...
    model = BiseNet(model_config, train_config, train_config['num_classes'], mode="train")
    model.build(num_gpus=configuration.num_gpus, reuse=tf.AUTO_REUSE)
    model_va = BiseNet(model_config, train_config, train_config['num_classes'], mode="validation")
    ...
    for step in range(start_step, total_steps):
      ...
      if step % 10 == 0:
        ...
        h, m = divmod(m, 60)
        format_str = ('%s: step %d, total loss = %.2f, (%.1f examples/sec; %.3f '
                      'sec/batch; %dh:%02dm:%02ds remains)')
        logging.info(format_str % (datetime.now(), step, loss, 
