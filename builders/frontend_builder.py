...
from frontends.nets.mobilenet import mobilenet_v3
from frontends.nets.mobilenet.mobilenet import training_scope
...
def build_frontend(inputs, frontend_config, is_training=True, reuse=False):
    ...
    if frontend == 'ResNet50':
        ...
        with slim.arg_scope(xception.xception_arg_scope()):
            ...
            init_fn = None
    elif frontend=="MobilenetV3":
        with slim.arg_scope(mobilenet_v3.training_scope(is_training=is_training)):
            logits, end_points = mobilenet_v3.mobilenet(inputs, is_training=is_training, scope='MobilenetV3', base_only=True, reuse=reuse)
            frontend_scope = 'MobilenetV3'
            init_fn = slim.assign_from_checkpoint_fn(model_path=os.path.join(pretrained_dir, 'model.ckpt-540000'),
                                                     var_list=slim.get_model_variables('MobilenetV3'),
                                                     ignore_missing_vars=True)
    elif frontend=="MobilenetV3-0.75":
        with slim.arg_scope(mobilenet_v3.training_scope(is_training=is_training)):
            logits, end_points = mobilenet_v3.mobilenet(inputs,depth_multiplier=0.75,is_training=is_training, scope='MobilenetV3',
                                                    base_only=True, reuse=reuse)
            frontend_scope = 'MobilenetV3'
            var_list = slim.get_model_variables('MobilenetV3')[:257]
            init_fn = slim.assign_from_checkpoint_fn(model_path=os.path.join(pretrained_dir, 'model.ckpt-220000'),
                                                     var_list=var_list,
                                                     ignore_missing_vars=True)
    elif frontend=="MobilenetV3-SMALL-1.0":
        with slim.arg_scope(mobilenet_v3.training_scope(is_training=is_training)):
            logits, end_points = mobilenet_v3.mobilenet(inputs,depth_multiplier=1,is_training=is_training, scope='MobilenetV3',
                                                    base_only=True,conv_defs=mobilenet_v3.V3_SMALL,reuse=reuse)
            frontend_scope = 'MobilenetV3'
            var_list = slim.get_model_variables('MobilenetV3')[:-7]
            init_fn = slim.assign_from_checkpoint_fn(model_path=os.path.join(pretrained_dir, 'model.ckpt-388500'),
                                                     var_list=var_list,
                                                     ignore_missing_vars=True)
    else:
