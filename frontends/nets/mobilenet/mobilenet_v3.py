...
import sys
sys.path.append("../bise-net/frontends")
...
def reduce_to_1x1(input_tensor, default_size=7, **kwargs):
  ...
  if h is not None and w == h:
    ...
    k = [min(h,default_size), min(w,default_size)]
...
def from_checkpoint_fn(endpoint,model_path):
    variables_restore = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='MobilenetV3')
    for v in tf.global_variables():
        if 'moving_mean' in v.name or 'moving_variance' in v.name:
            if "MobilenetV3" in v.name:
                variables_restore.append(v)
    for k,v in endpoint.items():
        print('mobile backbone output:',k,v)
    print(len(variables_restore))
    print (len(tf.global_variables()))
    variables_restore_n = [v for v in variables_restore if
                           'GN' not in v.name]  # Conv2d_1c_1x1 Bottleneck
    print(len(variables_restore_n))
    if variables_restore_n:
        saver = tf.train.Saver(variables_restore_n)
        #saver = tf_saver.Saver(variables_restore_n, reshape=reshape_variables)
        def callback(session):
            print('*'*10)
            print('load variables_restore_n%s',model_path)
