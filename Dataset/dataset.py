...
def _parse_function_avm(image_filename, label_filename, img_mean, class_dict):
    img_contents = tf.read_file(image_filename)
    label_contents = tf.read_file(label_filename)
...
def _parse_function_MVD(example_proto, split):
...
def _preprocess_image_MVD(sample, img_mean, is_training):
    ...
    """
    original_image, label = sample
    image = original_image
    # label = tf.Print(label, [tf.reduce_max(label), tf.reduce_min(label)], message='pre_process:::')
    image = tf.image.convert_image_dtype(image, dtype=tf.float32)
    if img_mean is not None:
        image -= img_mean/255

    # image = tf.Print(image, [tf.shape(image)], message='*******iamge:::', summarize=10)
    sample[0] = image

    # if not is_training:
    #   sample[0] = original_image
...
def _image_scaling(img, label):
    min_scale = 0.3
    max_scale = 1.0
    scale = tf.random_uniform([1], minval=min_scale, maxval=max_scale, dtype=tf.float32)
...
def _random_crop_and_pad_image_and_labels(image, label, crop_h, crop_w):
    ...
    pad_w = tf.maximum(crop_w, image_shape[1])-image_shape[1]
...
class DataLoader(object):
    def __init__(self, config, Dataset='CamVid', data_dir='./data/CamVid', class_dict='./CamVid/class_dict.csv'):
        self.config = config
    ...
    def prepare_data(self):
        ...
        if self.data_name == "CamVid":
          ...
          split_path_map = dict({"train":"%s/train_db.txt"%dataset_dir, "valid":"%s/test_db.txt"%dataset_dir})
          split_name = self.config['split']
          ...
          dataset = tf.data.Dataset.from_tensor_slices((image_file_list, label_file_list))
          dataset = dataset.map(lambda x, y: _parse_function_avm(x, y, img_mean, self.class_dict), num_parallel_calls=threads)
